...
export function crc8(dv) {
    let crc = 0;
    for (let p = 2; p < dv.byteLength - 2; p++) {
        crc = crc8_table[crc ^ dv.getUint8(p)];
    }
    return crc;
}

const dv = new DataView(new Uint8Array([0xEE, 0x06, 0x2D, 0xEE, 0xEA, 0x03, 0x04, 0x00, 199]).buffer);